...
본 프로젝트는 대규모 비즈니스 트랜잭션을 관리하는 SAP ERP 환경에서 자재 조달의 근간이 되는 **구매처(Vendor) 마스터 데이터와 구매오더(Purchase Order, PO)를 유기적으로 생성 및 조회**할 수 있도록 개발된 시스템입니다. 

실무 프로세스를 정교하게 반영하여 마스터 테이블 간의 데이터 연동 구조를 설계하였으며, 동적 스크린 변경 및 ALV Grid 이벤트 처리를 도입하여 수동 입력의 휴먼 에러를 최소화하고 트랜잭션 처리의 속도와 정확성을 대폭 개선하는 것을 목적으로 개발되었습니다.

---

## 🎯 개발 목표

- **구매처 관리 효율화**: 국내, 해외, 개인 등 계정그룹 조건에 따라 스크린 입력 필드를 동적으로 은닉/활성화하여 입력 데이터 오류 예방
...
- **ERP Platform**: SAP ECC / S/4HANA
- **Language**: ABAP
- **UI Framework**: SAP Dynpro (Screen Painter), CL_GUI_ALV_GRID (ALV Grid), CL_GUI_DOCKING_CONTAINER
- **Database**: SAP Dictionary (RDBMS)
...
- **구매처 마스터 동적 관리 (ZPROJECT18_001)**
  - 생성 및 조회 이원화 인터페이스 제공
  - 계정그룹(`KTOKK`) 선택 조건에 따른 개인번호, 사업자번호 입력창 제어 기능
  - 조회 결과 리스트상에서 실시간 사업자번호 수정 및 DB 영구 저장 기능
- **구매오더(PO) 라이프사이클 처리 (ZPROJECT18_002)**
  - 신규 PO 아이템 추가 시 PO 헤더 정보와 병렬 저장 기능 구성
...
- **플랫폼** : SAP GUI 기반 Desktop Environment
- **Device** : PC (SAP GUI Client 전용)

<div align="center">
  <img src="images/main_overview.png" alt="SAP 프로젝트 메인 화면" width="80%">
  <p><em>(PDF Page 1 - SAP 프로젝트 메인 인트로 화면)</em></p>
</div>
